...
class RAIDGroup(Base):
    ...
    def __repr__(self):
        repr_items = [self.raid_rid,self.raid_group_id,self.raid_type,self.total_size,self.free_size,self.highest_contig_free]
...
class Frame(Base):
    ...
    delete_date = Column('DeleteDate',DateTime)
...
class Host(Base):
    ...
    storage_group_wwn = Column('StorageGroup',Integer,ForeignKey('StorageGroup.SGWWN'))
    ...
    def __repr__(self):
        return "Host<'%s', '%s', '%s', ManReg: %s>" % (self.id,self.name,self.ip,str(self.manual_registration))
...
class LUN(Base):
    ...
    def __repr__(self):
        return "LUN<'%s','%s','%s','%s'>" % (self.wwn,str(self.alu),self.name,str(self.capacity))
...
class FrameSoftware(Base):
    ...
    def __repr__(self):
        return "FrameSoftware<'%s','%s','%s'>" % (self.name,self.rev,self.status)
...
class Drive(Base):
    ...
    def __repr__(self):
        return "Drive<'%s', '%s', '%s'>" % (self.location,self.drive_type,str(self.capacity))
